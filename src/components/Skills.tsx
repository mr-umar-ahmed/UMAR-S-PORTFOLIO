    ...
    <div className="w-full overflow-hidden py-2.5 flex select-none">
      <div className={`flex whitespace-nowrap gap-4 ${animateClass} hover:[animation-play-state:paused] transition-all`}>
        {listItems.map((item, idx) => (
          <div
            key={idx}
            className="px-6 py-3.5 bg-surface/30 glassmorphism rounded-full text-xs md:text-sm font-semibold tracking-wider font-display text-white/90 hover:text-accent hover:border-accent/40 transition-all duration-300 flex items-center gap-2"
          >
...
export default function Skills() {
  return (
    <section id="skills" className="relative py-24 bg-background overflow-hidden border-t border-white/5">
      ...
      <div className="w-full flex flex-col gap-2 relative z-10 py-6">
        <div className="absolute top-0 left-0 h-full w-16 md:w-40 bg-gradient-to-r from-background to-transparent z-20 pointer-events-none" />
        <div className="absolute top-0 right-0 h-full w-16 md:w-40 bg-gradient-to-l from-background to-transparent z-20 pointer-events-none" />
